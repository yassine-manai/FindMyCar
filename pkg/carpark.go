...
func AddCarpark(ctx context.Context, db *bun.DB, carpark *Carpark) error {
	log.Debug().Msgf("Adding Carpark....")

	_, err := db.NewInsert().Model(carpark).Exec(ctx)
	if err != nil {
...
func GetCarparkByID(ctx context.Context, db *bun.DB, id int) (*Carpark, error) {
	carpark := new(Carpark)

	log.Debug().Msgf("Fetching carpark  with ID: %d \n", id)

	err := db.NewSelect().Model(carpark).Where("id = ?", id).Scan(ctx)
...
func GetAllCarparks(ctx context.Context, db *bun.DB) ([]CarparkReposne, error) {
	var cprk []CarparkReposne
	err := db.NewSelect().Model(&cprk).Order("id ASC").Scan(ctx)
	if err != nil {
...
func GetAllCarparksExtra(ctx context.Context, db *bun.DB) ([]Carpark, error) {
	var carparks []Carpark
	err := db.NewSelect().Model(&carparks).Order("id ASC").Scan(ctx)
	if err != nil {
...
func UpdateCarpark(ctx context.Context, db *bun.DB, id int, updatedCarpark *Carpark) (int64, error) {
...
func DeleteCarpark(ctx context.Context, db *bun.DB, id int) (int64, error) {
	log.Debug().Msgf("Deleting Carpark with Id %v", id)

	result, err := db.NewDelete().Model((*Carpark)(nil)).Where("id = ?", id).Exec(ctx)
	if err != nil {
